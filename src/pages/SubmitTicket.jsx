      ...
      <aside className="hidden lg:flex w-64 h-full bg-white border-r border-gray-200 flex-col shadow-sm z-30 flex-none shrink-0">
        <div className="p-6 h-full flex flex-col">
          ...
          <div className="space-y-6 flex-1">
            <div>
               <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 px-1">Tickets</h4>
               <Link to="/" className="flex justify-between items-center text-sm font-bold text-[#123765] bg-[#123765]/5 px-3 py-2.5 rounded-lg border border-[#123765]/10">
                  <span>All</span>
                  <span className="bg-[#123765] text-white text-[10px] px-2 py-0.5 rounded-full">{tickets.length}</span>
               </Link>
            </div>
            ...
            <div>
               ...
               <div className="space-y-1">
                 <Link to="/" className="flex justify-between items-center text-sm font-bold text-[#1A2634] px-3 py-2.5 rounded-lg bg-gray-50 text-left">
                    <span>Submit Ticket</span>
                 </Link>
                 <Link to="/book-room" className="flex justify-between items-center text-sm font-medium text-gray-500 hover:text-[#1A2634] px-3 py-2.5 rounded-lg hover:bg-gray-50 transition-colors text-left">
      ...
      <main className="flex-1 h-full flex flex-col bg-[#f8fafc] overflow-hidden">
        ...
        <header className="px-6 lg:px-8 py-5 bg-white border-b border-gray-200 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
             {/* Mobile Sidebar Toggle (Placeholder) & Title */}
             <h1 className="text-2xl font-black text-[#1A2634]">Tickets</h1>
          </div>
        ...
        <div className="lg:hidden flex items-center gap-2 px-6 py-3 bg-[#f8fafc] border-b border-gray-200 overflow-x-auto shadow-inner custom-scrollbar shrink-0">
          <Link to="/" className="text-xs font-black text-[#123765] whitespace-nowrap bg-white px-4 py-2 rounded-lg border border-[#CCAA49]/40 shadow-sm flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#CCAA49]"></span>
            Submit Ticket
          </Link>
          <Link to="/book-room" className="text-xs font-bold text-gray-400 hover:text-[#1A2634] whitespace-nowrap px-4 py-2 transition-colors">
            Book a Room
          </Link>
        </div>

        {/* Filter Bar */}
        <div className="px-6 lg:px-8 py-3 bg-white border-b border-gray-200 flex justify-between items-center shrink-0 shadow-sm z-10 hidden sm:flex">
           <div className="flex items-center gap-4 text-[11px] font-bold text-gray-500">
             <span>Sort by:</span>
             <select className="border-none bg-gray-50 rounded-md py-1 px-2 text-[#1A2634] outline-none font-bold">
           ...
           <div className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
             Live Queue ({tickets.filter(t => t.status !== "Closed").length} Active)
           </div>
        </div>
          ...
          <div className="flex flex-col gap-4 w-full pb-12">
            {isLoading ? (
              ...
              <div className="py-24 text-center">
                <p className="text-gray-400 font-black text-sm uppercase tracking-widest mb-2 opacity-50">No Data</p>
              </div>
            ) : tickets
              ...
              .map((ticket, index) => (
                ...
                >
                  ...
                  <div className={`absolute left-0 top-0 bottom-0 w-1 ${colors[ticket.status]?.bg}`}></div>
                  ...
                  <div className="w-full md:w-32 flex flex-row md:flex-col items-center justify-between md:justify-center p-4 md:p-6 md:border-r border-gray-100 shrink-0 bg-gray-50/50">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider w-auto md:w-full text-center ${colors[ticket.status]?.bg} ${colors[ticket.status]?.text}`}>
                      {ticket.status}
                    </span>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center p-4 md:p-6 pb-2 md:pb-6">
                    <h3 className="font-black text-lg text-[#1A2634] truncate mb-3">{ticket.name}</h3>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Department</p>
                        <p className="text-sm font-bold text-[#123765] truncate">{ticket.department}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Phone Number</p>
                        <p className="text-sm font-medium text-gray-700">{ticket.phone}</p>
                      </div>
                      <div className="col-span-2 hidden lg:block">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Email</p>
                        <p className="text-sm font-medium text-gray-700 truncate">{ticket.email}</p>
                      </div>
                    </div>
                  </div>

                  {/* Description Column */}
                  <div className="w-full lg:w-[280px] shrink-0 p-4 md:p-6 pt-0 md:pt-6 md:border-l border-gray-100 flex items-center bg-gray-50/30">
                    <p className="text-sm text-gray-500 italic line-clamp-2 md:line-clamp-3">"{ticket.ticket_about}"</p>
                  </div>
        ...
        {isModalOpen && (
          ...
          >
            <motion.div 
              ...
              className="bg-white rounded-2xl shadow-2xl w-full max-w-[600px] overflow-hidden flex flex-col"
            >
              <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-[#f8fafc]">
                 <div>
                   <h2 className="text-2xl font-black text-[#1A2634] italic">Create Ticket</h2>
                   <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Incident Dispatch Portal</p>
                 </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto max-h-[75vh] custom-scrollbar">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-2">
                       <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Requester Name</label>
                    ...
                    <div className="flex flex-col gap-2">
                       ...
                       <input required type="tel" maxLength="11" className="modern-input" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value.replace(/\D/g, "") })} placeholder="09XXXXXXXXX" />
                    </div>
                 </div>

                 <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Email Address</label>
                    <input required type="email" className="modern-input" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="office@company.com" />
                 </div>

                 <div className="grid grid-cols-2 gap-6">
                    <div className="flex flex-col gap-2">
                       <label className="text-[10px] font-black text-[#123765] uppercase tracking-widest pl-1">Department</label>
                       <select className="modern-input cursor-pointer border-[#123765]/20 focus:border-[#123765]" value={formData.department} onChange={(e) => setFormData({ ...formData, department: e.target.value })}>
                    ...
                    <div className="flex flex-col gap-2">
                       <label className="text-[10px] font-black text-[#CCAA49] uppercase tracking-widest pl-1">Priority Target</label>
                       <select className="modern-input cursor-pointer border-[#CCAA49]/40 focus:border-[#CCAA49]" value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })}>
                 ...
                 <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Issue Description</label>
                    <textarea required rows="3" className="modern-input resize-none" value={formData.ticket_about} onChange={(e) => setFormData({ ...formData, ticket_about: e.target.value })} placeholder="Briefly describe the situation..." />
                 </div>
